...
internal_refresh (PSG * psg)
{
  ...
  {
    psg->base_incr =
      (uint32_t) ((double) psg->clk * (1 << GETA_BITS) / (8.0 * psg->rate));
  }
}
...
EMU2149_API void
PSG_init (PSG * psg, uint32_t c, uint32_t r)
{
...
EMU2149_API void
PSG_setFlags (PSG * psg, uint8_t flags)
{
...
EMU2149_API void
PSG_setStereoMask (PSG *psg, uint32_t mask)
{
  if(psg)
  {
    ...
    else
    {
      while (psg->realstep > psg->psgtime)
      {
        ...
        psg->sprev[1] = psg->snext[1];
          Pcalc_stereo (psg, psg->snext);
      }

      psg->psgtime -= psg->realstep;
        out[i] = (int32_t) (((double) psg->snext[0] * (psg->psgstep - psg->psgtime)
                         + (double) psg->sprev[0] * psg->psgtime) / psg->psgstep);
        out[i+1] = (int32_t) (((double) psg->snext[1] * (psg->psgstep - psg->psgtime)
